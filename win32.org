* pipe
** ConnectNamedPipe
Scenario	Does it wait?	How to know a client is connected
Synchronous (Default)	Yes (Blocks thread)	Function returns TRUE.
Asynchronous (Overlapped)	No (Returns instantly)	GetLastError() == ERROR_IO_PENDING; wait for the OVERLAPPED event.
Client connected too fast	No (Fails instantly)	GetLastError() == ERROR_PIPE_CONNECTED (Treat as success).

Even in synchronous mode, the function can fail immediately (return FALSE + ERROR_PIPE_CONNECTED) if a client connects in the tiny window between CreateNamedPipe and ConnectNamedPipe

** client connect deined
CreateNamedPipe时最后一个参数为null
When you pass nullptr (or NULL) as the last parameter to CreateNamedPipe, Windows applies the default security from the creating process. This frequently denies access to client processes (especially when the server runs as a service under SYSTEM/NetworkService and the client runs as a normal user).
这是客户端如果没有用管理员权限开启，就会出现error 5 (ERROR_ACCESS_DENIED)
* debug print
...
*** OutputDebugStringW with params
#define DEBUG_PRINT(fmt, ...) \
    { \
        WCHAR buf[512]; \
        swprintf(buf, 512, fmt, __VA_ARGS__); \
        OutputDebugString(buf); \
    }

// Usage:
DEBUG_PRINT(L"[SERVICE] ConnectNamedPipe failed with unexpected error %d\n", GetLastError());
...
When you **paint directly onto the screen ("single buffering")**, every action is immediately visible:

- You draw the background.
- You draw the text.
- You draw decorations.

If the process is slow, or involves several steps, the user might see partial updates (e.g. background drawn, then text appears, then highlight, etc). This is called **flicker**.

---

## The Solution: Double Buffering

**Double buffering** means you **draw everything into a memory-based image (off-screen)** first, then copy the final result in one step to the screen.

### Steps:

1. **Create a memory device context (DC)**
   This is an area in RAM where you can draw just like you would on the visible screen.

2. **Draw your graphics to the memory DC**
   All background and text goes to this “hidden canvas”, not the screen.

3. **Copy that finished image to the visible window**
   “Blitting” (“BitBlt”) is a fast operation that copies the result all at once.

**Result:**
The user only ever sees **the final product**, never the intermediate work, and flicker is eliminated.

---

## In Your Code

Your code implements double buffering as follows:

```cpp
HDC hdcMem = CreateCompatibleDC(hdc); // Create off-screen DC in memory
HBITMAP hbmMem = CreateCompatibleBitmap(hdc, width, height); // Make bitmap for drawing
HBITMAP hbmOld = (HBITMAP)SelectObject(hdcMem, hbmMem); // Attach bitmap to DC

// You draw everything to hdcMem—background, text, rectangle, etc.

// Finally, copy to visible DC in one fast operation:
BitBlt(hdc, rc.left, rc.top, width, height, hdcMem, 0, 0, SRCCOPY);

// Clean up memory DC and bitmap (important for resources)
SelectObject(hdcMem, hbmOld);
DeleteObject(hbmMem);
DeleteDC(hdcMem);
```

- **hdc:** The original windows-provided drawing context (on-screen).
- **hdcMem:** The off-screen memory context where you draw.
- **BitBlt:** Bulk copy from memory ("buffer") to the screen.

---

## Analogy

**Think of double buffering as painting on a sheet of paper, then instantly showing the complete painting to your audience**—instead of having them watch you draw each line as you go.

---

## Why It Matters for Owner-Drawn Controls

- Owner-drawn controls (like your ListView) require painting very frequently and may involve complex graphics.
- Without double buffering, windows may flicker badly, especially when resizing, scrolling, or selecting items quickly.

Double buffering is the canonical solution for **smooth UI drawing** in Win32 and other platforms.
