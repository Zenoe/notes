...
projectroot/module/main.cpp include "tool.h" --> flycheck(lsp) says can't find tool.h
solution:
in module/CMakeLists.txt, add
target_include_directories(main PRIVATE projectroot/common)
and run(*注意：一定要在build目录里 cmake build 之后*)
...
compile_commands.json 不管用

** compile_commands.json not work in my emacs
run in build
