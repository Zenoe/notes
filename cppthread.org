...
* job queue/ threadfun may be called frequently, often before the prevois call has finished
std::thread(...).detach() // bad

 * better pattern: Job Queues (Thread Pools)
A highly scalable pattern is the job queue:

One background thread (or a small pool) takes jobs off a queue and processes them.
Every call to threadfun just submits a job to the queue, not a new thread.
Example skeleton:
...
public:
    ...
    void threadfun(cb_type cb) {
        {
            std::lock_guard<std::mutex> lock(mutex_);
            tasks_.push([cb](){ cb(); }); // can use captures as needed
        }
        cv_.notify_one();
    }
};

// Every call to threadfun(cb) just adds a job (e.g. a callback) to the queue.
// Only ONE thread processes jobs, so there won't be dozens of threads.

// If You Actually Need Every Operation To Be Parallel
// If you really want a separate thread per call, you must:

// Control the maximum number of concurrent threads (e.g. with a thread pool)
// Make sure each thread does not access shared data unsafely. Use mutexes, etc.
// Realize user experience and performance may degrade as threads multiply.
#+end_src

* filter working thread with restart/cancle
#+begin_src cpp
class HistoryManager {
public:
    using FilterCallback = std::function<void()>; // Called on main thread after filtering?
    // You may need to adjust for context (pass result, etc.).
private:
    ...
    std::wstring pending_pattern_;
    FilterCallback pending_callback_;
    bool new_request_ = false;

    // ... (Your items_, filtered_items_, ... and their mutexes remain)
    ...
    HistoryManager() {
        filter_worker_ = std::thread([this]{
            ...
            while (!stop_) {
                ...
                auto pattern = pending_pattern_;
                auto callback = pending_callback_;
                new_request_ = false; // Mark current job as taken
                ...
                if (pattern.empty()) {
                    ...
                    // Filter: this->items_ to this->filtered_items_
                    std::vector<FileType> tmp; // or whatever container
                    {
                        std::lock_guard<std::mutex> lck(items_mtx);
                        for (const auto& item : this->items_) {
                            if (/* match item vs pattern */) tmp.push_back(item);
                        }
                    }
                    {
                        std::lock_guard<std::mutex> lck(filtered_items_mtx);
                        this->filtered_items_ = std::move(tmp);
                    }
                }

                // Optional: check for new request _here_ to abort result delivery
                lock.lock();
                if (!stop_ && !new_request_ && callback) {
                    lock.unlock();
