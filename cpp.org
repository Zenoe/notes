...
** find substr
///  split by four spaces
std::wstring delimiter = L"    "; // four spaces
size_t pos = 0;
size_t start = 0;
std::vector<std::wstring> tokens;

while ((pos = line.find(delimiter, start)) != std::wstring::npos) {
    tokens.push_back(line.substr(start, pos - start));
    start = pos + delimiter.length();
}
tokens.push_back(line.substr(start)); // Last token

// use regex to split by four spaces
#include <regex>

std::wregex re(L" {4}"); // four spaces
std::wsregex_token_iterator it(line.begin(), line.end(), re, -1);
std::wsregex_token_iterator end;
std::vector<std::wstring> tokens(it, end);
* references
** auto References
...
#+end_src
* share_ptr 操作
// Allocates the MyClass object and the reference counter in a single memory allocation (more efficient, avoids some thread-safety issues).
auto ptr1 = std::make_shared<MyClass>();

// Allocates the MyClass object and the control block separately (two allocations)
std::shared_ptr<MyClass> ptr2(new MyClass());
= auto ptr2 = std::shared_ptr<MyClass>(new MyClass());
// new的方式可以自定义deleter
std::shared_ptr<MyClass> ptr2(new MyClass(), my_deleter);
...
** &&
vec getVec() && { return std::move(v_); }
or vec && getVec() && { return std::move(v_); }

✅ vec&& getVec() && { return std::move(v_); }   // correct
This is a ref-qualified member function where:

vec&& is the return type (rvalue reference)

&& after the parameter list is the ref-qualifier, meaning this function can only be called on rvalue objects (temporary objects or objects being moved from)
The first version you showed (vec getVec() && { return std::move(v_); }) would work but returns by value, which would involve an extra copy/move. Returning vec&& is more efficient as it directly returns an rvalue reference to the moved-from object.
...
private:
    vec v_;
public:
    // For lvalue objects - returns lvalue reference
    vec& getVec() & { return v_; }

    // For rvalue objects - returns rvalue reference after moving
    vec&& getVec() && { return std::move(v_); }
};

// Usage:
MyClass obj;
vec& ref = obj.getVec();           // calls lvalue version
vec&& rref = std::move(obj).getVec(); // calls rvalue version
The ref-qualifier && ensures this overload is only called when the object itself is an rvalue, which is typically when you want to move from its members.
...
// ok manul copy
ProxyStatsSnapshot get_stats() const {
    return {
...
int main() {
    ...
    // 2. 在这片自定义内存上构造对象
    // placement new call the ctor of Foo at that location. you do not allocate memory; you only construct the object.
...
block 是一个 void* 指针类型，转换成 char* 是为了后续按字节步进地处理内存（比如当前你写的 current += chunk_size_）。
使用 static_cast 是比较安全的，如果 block 是 void* 或从别的 char* 转过来的指针，这样转换没有问题。

reinterpret_cast
用于任何指针类型之间的强制转换，或者指针和整数类型之间的转换。
没有类型安全检查，完全是逐位解释。
常用于底层代码，比如将字节缓冲区转成某种结构体。

是将一个 char* 内存指针（只知道它是指向某块内存）强制转换为 Chunk*（结构体指针）。这样做是告诉编译器“从这里开始，把这块内存看作一个 Chunk 对象”。
这种用法适合底层内存管理、对象池、自己分配内存场景，但有一定风险，如果类型、内存对齐不对就会出错。

| 特性       | static_cast  | reinterpret_cast |
| 类型检查   | 有           | 无               |
| 安全性     | 相对安全     | 危险             |
| 使用场景   | 相关类型转换 | 无关类型重新解释 |
| 编译时检查 | 有           | 无               |

- Standard Conversions: int to double, double to int, enum to int.

- Pointer Conversions: Converting between related pointer types, specifically upcasts in a class hierarchy (converting a derived class pointer to a base class pointer).

- void* to any pointer type: void* is a generic pointer. static_cast can convert it back to a specific type.

- Explicitly calling constructors or conversion operators.
  #+begin_src cpp
    MyString s1 = "Hello";  // const char* implicitly converted to MyString
    displayString("World"); // const char* implicitly converted to MyString

    // EXPLICIT conversion using static_cast
    MyString s2 = static_cast<MyString>("Explicit");

  #+end_src
** dynamic_cast  runtime check
downcast.  把指向基类的指针转化为指向子类的指针，这样可以执行子类的函数
#+begin_src cpp
    ...
    if (derivedPtr) { // Check if the cast succeeded
        derivedPtr->derivedMethod(); // This will run
    } else {
        std::cout << "Cast failed!\n";
    }

    Base* basePtr2 = new Base; // Base pointer to a Base object

    // This downcast will fail
...
Removing const to call a legacy C function that takes a non-const pointer but doesn't modify the data.
void legacyPrint(char* str) {
    std::cout << str << std::endl;
}

void modernPrint(const char* str) {
    ///  /legacyPrint(str); // Error! Can't convert 'const char*' to 'char*'
    legacyPrint(const_cast<char*>(str)); // OK, we promise it's safe
}
...
作用：声明一个可变参数模板，表示 Args 是一个模板参数包（template parameter pack）

含义：可以接受任意数量、任意类型的模板参数

2. void emplace_back(Args&&... args)
作用：将模板参数包 Args 展开为函数参数包 args

含义：函数接受与模板参数数量对应的右值引用参数

示例：如果调用 emplace_back(1, 2.0, "hello")，则：
...
作用：参数包展开（parameter pack expansion）

含义：将参数包 args 中的每个参数分别用 std::forward 转发
...
* 空对象传递
函数参数是一个对象，传递空对象 fun(const Obj& obj){}
✅fun({}) 它会调用 Obj 的默认构造函数来创建一个临时对象，然后绑定到 const 左值引用上。要求：Obj 必须是可默认构造的（Obj() 可用）
✅fun(Obj()) OK

对简单对象时 qt 里可以 emit loginFinished({});
emit loginFinished(Obj()) ✔️️

在声明变量时，Obj obj() 会被解析为函数声明，而不是对象定义：

// ❌ 错误：这不是对象定义，而是函数声明
Obj obj();   // 声明了一个返回 Obj、无参数的函数 obj

// ✅ 正确的方式
Obj obj1;    // 默认构造
Obj obj2{};  // C++11 起，值初始化

* 定义变量被认为是函数声明
QNetworkRequest request(QUrl(url));  // 编译错误
QUrl qurl(url);
QNetworkRequest request; // 显式默认构造 // OK
